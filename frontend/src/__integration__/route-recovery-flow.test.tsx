...
// Fork note: /dashboard renders the fork dashboard page, so it must fail
// alongside upstream's DashboardPage for the reload-recovery cases below.
vi.mock("@/features/fork/dashboard/components/fork-dashboard-page", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/features/fork/dashboard/components/fork-dashboard-page")>();
  const ForkDashboardPage = actual.ForkDashboardPage;
...
describe("route recovery flow integration", () => {
  ...
  it("preserves dashboard search input across healthy same-path URL updates", async () => {
    const user = userEvent.setup({ delay: null });
    // Fork note: upstream's dashboard (with the search box) lives at /upstream-dashboard.
