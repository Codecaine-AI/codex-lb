      ...
      asUseReportsResult({
        data: {
          ...
          byModel: filters.model
            ? [{ model: "gpt-5.1", costUsd: 1, requests: 1, percentage: 100, tokens: 0 }]
            : [
                { model: "gpt-5.1", costUsd: 1, requests: 1, percentage: 50, tokens: 0 },
                { model: "gpt-5.2", costUsd: 1, requests: 1, percentage: 50, tokens: 0 },
              ],
    ...
    useReportsMock.mockImplementation((filters) =>
      asUseReportsResult({
        data: {
          ...EMPTY_REPORT,
          byModel: [
            { model: "gpt-5.1", costUsd: 1, requests: 1, percentage: 100, tokens: 10 },
          ],
  ...
  it("renders an independent user-agent distribution card below the model card", async () => {
    useReportsMock.mockImplementation(() =>
      asUseReportsResult({
        data: {
          ...EMPTY_REPORT,
          byModel: [{ model: "gpt-5.1", costUsd: 12, requests: 3, percentage: 100, tokens: 300 }],
          byUseragent: [
    ...
    useReportsMock.mockImplementation(() =>
      asUseReportsResult({
        data: {
          ...EMPTY_REPORT,
          byModel: [
            { model: "gpt-5.1", costUsd: 12, requests: 3, percentage: 100, tokens: 300 },
          ],
          byUseragent: [
            { useragent: "CLI", costUsd: 10, requests: 2, percentage: 100 },
          ],
